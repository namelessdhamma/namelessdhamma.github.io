...
# V38: Safe Tool Broker execution layer. Models never receive credentials; unknown tools are denied.
broker_code=r"""
...
def nd_gateway_self_probe():
    time.sleep(18)
    uid=-370037
    q='Проверь в интернете последнюю официальную запись в release notes ChatGPT от OpenAI. Используй актуальные источники и назови дату.'
    try:
        out=routed_response(uid,q)
        print('ND_GATEWAY_SELF_PROBE',json.dumps({'ok':True,'chars':len(out or ''),'last_route':state.get('last_route'),'last_provider':state.get('last_provider'),'openrouter_model':state.get('last_openrouter_model'),'mutations':False},ensure_ascii=False),flush=True)
    except Exception as e:
        print('ND_GATEWAY_SELF_PROBE',json.dumps({'ok':False,'error':cleanerr(e),'mutations':False},ensure_ascii=False),flush=True)
    finally:
        history_by_uid.pop(uid,None);mode_by_uid.pop(uid,None)

"""
def_marker='insert_before="def heuristic_route(text):\\n"'
if def_marker not in code:raise RuntimeError('V38 insert_before marker not found')
...
if needle2 not in code:raise RuntimeError('V38 composed read-only injection marker not found')
...
if route_assign not in code:raise RuntimeError('V38 original_text marker not found')
code=code.replace(route_assign,route_assign+chr(10)+override_code,1)

route_line="    ndctx=nd_read_context(original_text,route)"
if route_line not in code:raise RuntimeError('V38 ndctx line not found')
...
if_line="    if ndctx:"
if if_line not in code:raise RuntimeError('V38 if ndctx line not found')
...
if text_line not in code:raise RuntimeError('V38 ndctx text line not found')
...
# Autonomous E2E qualification: no VK message and no external mutation.
thread_marker="threading.Thread(target=startup,daemon=True).start()"
if thread_marker not in code:raise RuntimeError('V38 startup thread marker not found')
code=code.replace(thread_marker,thread_marker+chr(10)+"threading.Thread(target=nd_gateway_self_probe,daemon=True).start()",1)
...
code=code.replace("ND_VK_GATEWAY_V13_ND_READONLY_START","ND_VK_GATEWAY_V38_SAFE_TOOL_BROKER_START",1)
'''
outer=outer.replace(marker,marker+"\n"+extra,1)
outer=outer.replace("ND_VK_GATEWAY_V23_ND_READONLY_RANKED_START","ND_VK_GATEWAY_V38_SAFE_TOOL_BROKER_RANKED_START",1)
outer=outer.replace("ND_V23_BOOTSTRAP","ND_V38_BOOTSTRAP",1)
outer=outer.replace("ND_V23_OUTER","ND_V38_OUTER",1)
outer=outer.replace("nd_vk_gateway_v23_outer.py","nd_vk_gateway_v38_outer.py",1)
print('ND_V38_WRAPPER_READY',flush=True)
exec(compile(outer,'nd_vk_gateway_v38_loader.py','exec'))
