...
if src.count(load_marker)!=1:
    raise RuntimeError('V2 linear bridge load marker not found exactly once')
...
new_gateway="GATEWAY_URL='https://raw.githubusercontent.com/namelessdhamma/namelessdhamma.github.io/277a7f08f780cda8206a32e4f7690c9eecffcec6/tmp/nd_vk_gateway_v15_father_min_profile.py'"

patch = (
    "old_gateway="+repr(old_gateway)+"\n"
    "new_gateway="+repr(new_gateway)+"\n"
    "if s.count(old_gateway)!=1: raise RuntimeError('V2 active V14 gateway marker not found exactly once')\n"
    "s=s.replace(old_gateway,new_gateway,1)\n"
    "print('ND_FATHER_MIN_PROFILE_ROUTE_PATCH_READY',flush=True)\n"
)

src=src.replace(load_marker,load_marker+patch,1)
print('ND_LINEAR_BRIDGE_V2_FATHER_PROFILE_WRAPPER_READY',flush=True)
exec(compile(src,'nd_gateway_linear_bridge_v2_father_profile.py','exec'))
