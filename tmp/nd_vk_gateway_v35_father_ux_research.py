...
# V35: Safe Tool Broker execution layer. Models never receive credentials; unknown tools are denied.
broker_code=r"""
...
"""
def_marker='insert_before="def heuristic_route(text):\\n"'
if def_marker not in code:raise RuntimeError('V35 insert_before marker not found')
...
if needle2 not in code:raise RuntimeError('V35 composed read-only injection marker not found')
...
if route_assign not in code:raise RuntimeError('V35 original_text marker not found')
code=code.replace(route_assign,route_assign+chr(10)+override_code,1)

route_line="    ndctx=nd_read_context(original_text,route)"
if route_line not in code:raise RuntimeError('V35 ndctx line not found')
...
if_line="    if ndctx:"
if if_line not in code:raise RuntimeError('V35 if ndctx line not found')
...
if text_line not in code:raise RuntimeError('V35 ndctx text line not found')
...
cmd_new=cmd_old+chr(10)+r"""                if text in ('/ктоя','/id'):
...
code=code.replace("ND_VK_GATEWAY_V13_ND_READONLY_START","ND_VK_GATEWAY_V35_SAFE_TOOL_BROKER_START",1)
'''
outer=outer.replace(marker,marker+"\n"+extra,1)
outer=outer.replace("ND_VK_GATEWAY_V23_ND_READONLY_RANKED_START","ND_VK_GATEWAY_V35_SAFE_TOOL_BROKER_RANKED_START",1)
outer=outer.replace("ND_V23_BOOTSTRAP","ND_V35_BOOTSTRAP",1)
outer=outer.replace("ND_V23_OUTER","ND_V35_OUTER",1)
outer=outer.replace("nd_vk_gateway_v23_outer.py","nd_vk_gateway_v35_outer.py",1)
print('ND_V35_WRAPPER_READY',flush=True)
exec(compile(outer,'nd_vk_gateway_v35_loader.py','exec'))
