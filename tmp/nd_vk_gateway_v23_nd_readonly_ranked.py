...
new_score="""        name=(item.get('name') or p.rsplit('/',1)[-1]).lower()
        ...
        cand.append((score,p,item))
"""
if old_score not in code: raise RuntimeError('V23 scoring block not found')
...
code=code.replace("ND_VK_GATEWAY_V17_ND_READONLY_COMPLETE_START","ND_VK_GATEWAY_V23_ND_READONLY_RANKED_START",1)
'''
outer=outer.replace(marker,inject+"\nprint('ND_V23_BOOTSTRAP',flush=True)",1)
print('ND_V23_OUTER',flush=True)
exec(compile(outer,'nd_vk_gateway_v23_outer.py','exec'))
