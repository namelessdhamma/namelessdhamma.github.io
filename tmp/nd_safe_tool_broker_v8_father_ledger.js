...
const SB_REV="51e5f29e67619ef5af8a9f5876b009740e6aa58c";
const SB_BASE="https://raw.githubusercontent.com/namelessdhamma/namelessdhamma.github.io/"+SB_REV+"/tmp/";
const sbFrag=await (await fetch(SB_BASE+"nd_father_ledger_sandbox_v1.jsfrag")).text();
const sbProbe=await (await fetch(SB_BASE+"nd_father_ledger_selfprobe_v1.jsfrag")).text();
...
if(!patched.includes(sbInvokeMarker))throw new Error("ledger invoke marker missing");
...
if(!patched.includes(sbParseOld))throw new Error("ledger handler parse marker missing");
patched=patched.replace(sbParseOld,sbParseNew);
if(!patched.includes("const result=await invokeTool(tool,q);"))throw new Error("ledger invoke call marker missing");
patched=patched.replace("const result=await invokeTool(tool,q);","const result=await invokeTool(tool,q,args);");
patched=patched.replace('service:"ND Safe Tool Broker v2 + Google Authority",mode:"READ_ONLY",mutations:false','service:"ND Safe Tool Broker v8 + Father Ledger Sandbox",mode:"BOUNDED_WRITE_SANDBOX",canonical_mutations:false,sandbox_mutations:true');
patched=patched.replace('console.log("ND_SAFE_TOOL_BROKER_START",JSON.stringify({port:PORT,statehead_id:STATEHEAD_ID,google:!!CLIENT_EMAIL&&!!PRIVATE_KEY_B64,github:!!GITHUB_TOKEN,web:!!GROQ_API_KEY,mutations:false}));','console.log("ND_SAFE_TOOL_BROKER_V8_FATHER_LEDGER_START",JSON.stringify({port:PORT,statehead_id:STATEHEAD_ID,google:!!CLIENT_EMAIL&&!!PRIVATE_KEY_B64,github:!!GITHUB_TOKEN,web:!!GROQ_API_KEY,canonical_mutations:false,sandbox_mutations:true,father_root:FATHER_ROOT_ID,storage:"OWNER_OWNED_APPEND_ONLY_LEDGERS"}));');
const sbServe="Bun.serve({port:PORT,fetch:handler});";
if(!patched.includes(sbServe))throw new Error("ledger serve marker missing");
