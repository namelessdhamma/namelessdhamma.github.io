...
BASE='https://raw.githubusercontent.com/namelessdhamma/namelessdhamma.github.io/b95d3e6209c7d9dcd96e44b9c4f92081986b297a/tmp/nd_vk_gateway_v16e_resilience_memory.py'
...
v17_patch_code=r"""
# V17: OmniRoute private-network reserve route. It is an auxiliary fallback, never an authority.
...
if _env_marker not in src: raise RuntimeError('V17 OmniRoute env marker missing')
src=src.replace(_env_marker,_env_new,1)

_omni_func=r"""
def omniroute_chat(messages,max_tokens=3000,temperature=0.35):
    if not OMNIROUTE_BASE_URL: raise RuntimeError('OMNIROUTE_BASE_URL missing')
    if _provider_blocked('omniroute'): raise RuntimeError('OmniRoute circuit open')
    ...
    try:
        ...
        _provider_fail('omniroute',e); raise
    ch=j.get('choices') or []
    if not ch: raise RuntimeError('OmniRoute returned no choices')
    out=((ch[0].get('message') or {}).get('content') or '').strip()
    if not out: raise RuntimeError('OmniRoute returned empty content')
...
if _reserve_marker not in src: raise RuntimeError('V17 reserve marker missing')
...
_probe_new="""            else:
            ...
            return
"""
if _probe_tail not in src: raise RuntimeError('V17 startup probe marker missing')
