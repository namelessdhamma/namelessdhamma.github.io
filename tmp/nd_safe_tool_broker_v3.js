...
const TOOL_CATALOG = {
  ...
  broker_executable:[
    ...
    {name:"web_current",description:"Current source-backed web research through Groq Compound/Compound Mini with bounded fail-closed retrieval."},
...
async function webCurrent(q){
  ...
  const compact=raw.slice(0,1800);
  const system="Today is "+today+". You are a READ-ONLY web evidence retriever. Use web_search for every request. Use visit_website only when needed to verify a primary/official page. Prefer official/primary sources and recent dated material. Return a compact factual brief, preserving direct source URLs and dates. Do not answer current claims from model memory. If evidence is insufficient, say so.";
  const variants=[
    {model:GROQ_RESEARCH_MODEL||"groq/compound",query:compact,max:1700,tools:["web_search","visit_website"]},
    {model:"groq/compound-mini",query:compact.slice(0,1000),max:1200,tools:["web_search"]},
    {model:"groq/compound",query:compact.slice(0,800),max:1200,tools:["web_search"]}
  ];
  let lastErr="";
  for(const v of variants){
    const payload={model:v.model,messages:[
      {role:"system",content:system},
      {role:"user",content:v.query}
    ],max_completion_tokens:v.max,compound_custom:{tools:{enabled_tools:v.tools}}};
    for(let attempt=0;attempt<2;attempt++){
      const r=await fetch("https://api.groq.com/openai/v1/chat/completions",{method:"POST",headers:{Authorization:"Bearer "+GROQ_API_KEY,"Content-Type":"application/json","Groq-Model-Version":"latest","User-Agent":"nd-safe-tool-broker/3.0"},body:JSON.stringify(payload)});
      const t=await r.text();
      if(r.ok){
        const j=JSON.parse(t),msg=j.choices?.[0]?.message||{},exec=msg.executed_tools||[];
        if(!exec.length){lastErr="web research used no tools";break;}
        const urls=[];
        const rawSources=JSON.stringify(exec)+" "+String(msg.content||"");
        for(const m of rawSources.matchAll(/https?:\/\/[^\s"\\<>]+/g)){
          const u=m[0].replace(/[).,;\]]+$/,"");
          if(!urls.includes(u))urls.push(u);
          if(urls.length>=12)break;
        }
        return {brief:String(msg.content||"").trim().slice(0,12000),source_urls:urls,tool_calls:exec.length,model:v.model,query_chars:v.query.length,mutations:false};
      }
      lastErr="groq "+r.status+": "+t.slice(0,350);
      if(r.status===429&&attempt===0){
        const retry=Math.min(5,Math.max(1,Number(r.headers.get("retry-after")||2)));
  ...
  throw new Error(lastErr||"web research failed");
}
...
async function handler(req){
  ...
  if(u.pathname==="/health")return Response.json({ok:true,service:"ND Safe Tool Broker v3 + Google Authority",mode:"READ_ONLY",mutations:false});
