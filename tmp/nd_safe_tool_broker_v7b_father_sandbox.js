...
if(!patched.includes(sbParseOld))throw new Error("sandbox handler parse marker missing");
patched=patched.replace(sbParseOld,sbParseNew);
if(!patched.includes("const result=await invokeTool(tool,q);"))throw new Error("sandbox invoke call marker missing");
patched=patched.replace("const result=await invokeTool(tool,q);","const result=await invokeTool(tool,q,args);");
patched=patched.replace('service:"ND Safe Tool Broker v2 + Google Authority",mode:"READ_ONLY",mutations:false','service:"ND Safe Tool Broker v7b + Father Sandbox",mode:"BOUNDED_WRITE_SANDBOX",canonical_mutations:false,sandbox_mutations:true');
patched=patched.replace('console.log("ND_SAFE_TOOL_BROKER_START",JSON.stringify({port:PORT,statehead_id:STATEHEAD_ID,google:!!CLIENT_EMAIL&&!!PRIVATE_KEY_B64,github:!!GITHUB_TOKEN,web:!!GROQ_API_KEY,mutations:false}));','console.log("ND_SAFE_TOOL_BROKER_V7B_FATHER_SANDBOX_START",JSON.stringify({port:PORT,statehead_id:STATEHEAD_ID,google:!!CLIENT_EMAIL&&!!PRIVATE_KEY_B64,github:!!GITHUB_TOKEN,web:!!GROQ_API_KEY,canonical_mutations:false,sandbox_mutations:true,father_root:FATHER_ROOT_ID}));');
