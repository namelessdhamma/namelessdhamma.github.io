...
_new_classify="""def classify_route(text):
    ...
    try:
"""
if _old_classify not in src:
    raise RuntimeError('V15 classify_route marker not found')
...
inject_into_v13 = (
    ...
    "if src.count(v9_load_marker)!=1: raise RuntimeError('V15 V9 load marker not found exactly once')\n"
...
new_marker="src=src.replace('ND_VK_GATEWAY_V13_INDEPENDENT_WEB_START','ND_VK_GATEWAY_V15_FATHER_MIN_PROFILE_START',1)"
if old_marker not in outer:
    raise RuntimeError('V15 startup marker not found')
outer=outer.replace(old_marker,new_marker,1)

print('ND_V15_FATHER_MIN_PROFILE_WRAPPER_READY',flush=True)
exec(compile(outer,'nd_vk_gateway_v15_father_min_profile.py','exec'))
