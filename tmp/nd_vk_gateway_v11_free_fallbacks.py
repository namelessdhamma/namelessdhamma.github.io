...
("ND_VK_GATEWAY_V9_FREE_ROUTER_START","ND_VK_GATEWAY_V11_FREE_FALLBACKS_START")
]
for old,new in repls:
    if old not in src:
        raise RuntimeError('Expected V9 pattern not found: '+old[:80])
    src=src.replace(old,new,1)
exec(compile(src,'nd_vk_gateway_v11_free_fallbacks.py','exec'))
