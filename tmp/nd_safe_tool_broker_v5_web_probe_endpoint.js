...
const compact=String.raw`async function webCurrent(q){
  ...
  ],max_completion_tokens:1200,compound_custom:{tools:{enabled_tools:["web_search","visit_website"]}}};
  async function callOnce(){const r=await fetch("https://api.groq.com/openai/v1/chat/completions",{method:"POST",headers:{Authorization:"Bearer "+GROQ_API_KEY,"Content-Type":"application/json","Groq-Model-Version":"latest","User-Agent":"nd-safe-tool-broker/5.0"},body:JSON.stringify(payload)});const t=await r.text();return {r,t};}
  let {r,t}=await callOnce();if(r.status===429){await new Promise(res=>setTimeout(res,1200));({r,t}=await callOnce());}
...
const old='if(u.pathname==="/health")return Response.json({ok:true,service:"ND Safe Tool Broker v2 + Google Authority",mode:"READ_ONLY",mutations:false});\n  if(!authorized(req))return new Response("unauthorized",{status:401});';
const add=String.raw`if(u.pathname==="/health")return Response.json({ok:true,service:"ND Safe Tool Broker v5 qualification",mode:"READ_ONLY",mutations:false});
  if(u.pathname==="/probe/web"&&req.method==="GET"){
    try{
      const r=await webCurrent("Find the latest official OpenAI ChatGPT release-notes update. Use official OpenAI source only. Return direct URL and date.");
      return Response.json({ok:true,verified_search:r.verified_search,tool_calls:r.tool_calls,source_urls:r.source_urls.slice(0,4),brief:String(r.brief||"").slice(0,1200),mutations:false});
    }catch(e){return Response.json({ok:false,error:String(e).slice(0,500),mutations:false},{status:502});}
  }
  if(!authorized(req))return new Response("unauthorized",{status:401});`;
if(!src.includes(old))throw new Error('handler auth marker not found');
src=src.replace(old,add);
