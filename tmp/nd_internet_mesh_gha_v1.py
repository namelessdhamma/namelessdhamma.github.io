import hashlib, json, os, sys, time, urllib.parse, urllib.request, uuid
...
PROVIDERS = {
    ...
    "tinyfish": {"url": os.getenv("ND_TINYFISH_MCP_URL","https://agent.tinyfish.ai/mcp"),"token":os.getenv("TINYFISH_API_KEY","").strip(),"header":"X-API-Key","prefix":"","allow":{"search","fetch_content","run_web_automation","get_run","list_runs","cancel_run","get_wallet"}},
}
...
def post(provider,payload,sid=None,timeout=100):
    ...
    h={"Accept":"application/json, text/event-stream","Content-Type":"application/json","User-Agent":"ND-GitHub-Failover/1.2"}
    h[cfg["header"]]=cfg["prefix"]+cfg["token"]
    if provider=="tinyfish":h.update({"X-TF-Request-Origin":"nd-github-failover","X-TF-Client-Name":"nd-github-failover","X-TF-Client-Version":"1.2.0"})
...
def initialize(provider):
    status,h,raw=post(provider,{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-06-18","capabilities":{},"clientInfo":{"name":"ND GitHub Failover","version":"1.2.0"}}},timeout=45)
...
def tool_value(msg):
    ...
    if r.get("isError"):raise RuntimeError("provider_tool_error:"+clean(r.get("content") or r))
...
def request_uuid(request_key):
    b=bytearray(hashlib.sha256(request_key.encode("utf-8")).digest()[:16])
    b[6]=(b[6]&0x0f)|0x40
    b[8]=(b[8]&0x3f)|0x80
...
def kernel_smoke(request_key):
    ...
    try:
        ...
        try:
            call_tool("kernel","manage_browsers",{"action":"delete","session_id":sid,"context":"Deleting the temporary failover qualification browser immediately after verification so no orphan browser session remains."})
        except Exception as e:
            raise RuntimeError("kernel_smoke_cleanup_failed:"+clean(e))

def tinyfish_smoke(request_key):
    if not request_key:raise RuntimeError("request_key_required_for_smoke")
    session_id=request_uuid(request_key)
    ...
    for attempt in range(7):
        state=str(deep_find(last,"status") or deep_find(last,"state") or "").upper()
        text=json.dumps(last,ensure_ascii=False)
        if state in ("COMPLETED","SUCCESS","SUCCEEDED","DONE") or ("Example Domain" in text and state not in ("FAILED","ERROR","CANCELLED","CANCELED")):
            if "Example Domain" not in text:raise RuntimeError("tinyfish_smoke_content_mismatch:"+clean(last))
            return {"ok":True,"provider":"tinyfish","scenario":"single_idempotent_run","session_id":session_id,"run_id_present":bool(run_id),"content_verified":True}
        if state in ("FAILED","ERROR","CANCELLED","CANCELED"):raise RuntimeError("tinyfish_smoke_terminal_"+state.lower()+":"+clean(last))
        if not run_id:
            run_id=deep_find(last,"run_id") or deep_find(last,"id")
            if not run_id:break
        time.sleep(5 if attempt<2 else 10)
        last=call_tool("tinyfish","get_run",{"id":run_id},60)
    if run_id:
        try:call_tool("tinyfish","cancel_run",{"id":run_id},45)
        except Exception:pass
    raise RuntimeError("tinyfish_smoke_timeout_or_unresolved:"+clean(last))
...
def main():
    ...
    if provider=="status":
        out={"ok":True,"service":"ND GitHub Internet Failover","version":"1.2.0-qualification","providers":{k:{"configured":bool(v["token"]),"upstream":v["url"],"allowlisted_tools":len(v["allow"])} for k,v in PROVIDERS.items()},"secrets_returned":False}
    else:
        ...
        if tool=="__smoke__":
            ...
            out["retry_policy"]="NO_NEW_START_ON_AMBIGUOUS_RESPONSE_REUSE_SAME_REQUEST_KEY"
            ...
            if provider=="tinyfish" and tool=="run_web_automation" and request_key and "session_id" not in args:
                args=dict(args);args["session_id"]=request_uuid(request_key)
            init,msg=request(provider,"tools/call",{"name":tool,"arguments":args},150)
            out={"ok":True,"provider":provider,"tool":tool,"serverInfo":((init.get("result") or {}).get("serverInfo") or {}),"response":msg,"request_key":request_key or None,"retry_policy":"NO_NEW_START_ON_AMBIGUOUS_RESPONSE_REUSE_SAME_REQUEST_KEY" if sensitive else "BOUNDED_RETRY_AFTER_STATUS_CHECK"}
