...
state={'ok':True,'phase':'starting','token_present':bool(TOKEN),'groq_present':bool(GROQ_API_KEY),'provider':'groq','model':GROQ_MODEL,'group_id':None,'callback_server_id':None,'callback_configured':False,'groq_probe':None,'last_error':None,'last_event_type':None,'last_event_at':None,'ai_calls':0}
history_by_uid={}
seen=set();seen_order=[]
last_call={}
lock=threading.Lock()
...
def groq_chat(messages,reasoning='medium',max_tokens=1200):
    if not GROQ_API_KEY: raise RuntimeError('GROQ_API_KEY missing')
    payload={'model':GROQ_MODEL,'messages':messages,'reasoning_effort':reasoning,'max_completion_tokens':max_tokens}
    req=Request('https://api.groq.com/openai/v1/chat/completions',data=json.dumps(payload,ensure_ascii=False).encode('utf-8'),method='POST',headers={'Content-Type':'application/json','Authorization':'Bearer '+GROQ_API_KEY})
    try:
        with urlopen(req,timeout=90) as r:j=json.loads(r.read().decode('utf-8'))
    except HTTPError as e:
        try: body=e.read().decode('utf-8','replace')
        except Exception: body=''
        raise RuntimeError('Groq HTTP %s: %s'%(e.code,cleanerr(body)))
    except Exception as e:
        raise RuntimeError('Groq request failed: '+cleanerr(e))
    choices=j.get('choices') or []
    if not choices: raise RuntimeError('Groq returned no choices: '+cleanerr(j))
...
def ai_response(uid,text):
    ...
    out=groq_chat(msgs,'medium',1200)
    history_by_uid[uid]=(hist+[{'role':'user','content':text[:6000]},{'role':'assistant','content':out[:6000]}])[-8:]
    state['ai_calls']+=1
    return out[:3900]
...
def register():
    ...
    for s in servers(gid):
        if (s.get('url') or '').rstrip('/')==CALLBACK_URL.rstrip('/'):
            sid=int(s['id']);break
    if sid is None:
        r=vk('groups.addCallbackServer',{'group_id':gid,'url':CALLBACK_URL,'title':'ND Gateway5 Groq'})
...
def startup():
    ...
    for wait in (1,3,8,15,30):
        ...
        try:
            ...
            try:
                p=groq_chat([{'role':'user','content':'Reply only with OK.'}],'low',16)
                state['groq_probe']='ok';print('GROQ_PROBE_OK',json.dumps({'reply':p[:40]}),flush=True)
            except Exception as pe:
                state['groq_probe']='error';state['last_error']=cleanerr(pe);print('GROQ_PROBE_ERROR',state['last_error'],flush=True)
            return
        except Exception as e:
...
def send(peer,text):
    ...
    print('VK_SENT',json.dumps({'peer_id':peer,'result':r}),flush=True);return r
    ...
    def do_POST(self):
        ...
        def reply():
            try:
                ...
                if text=='/status':send(peer,'ND VK Gateway: VK=OK; AI=%s; provider=Groq; model=%s; probe=%s'%('OK' if GROQ_API_KEY else 'NOT CONFIGURED',GROQ_MODEL,state.get('groq_probe')));return
                ...
                if not GROQ_API_KEY:send(peer,'VK-контур работает. Для бесплатного AI-контура нужен GROQ_API_KEY.');return
                now=time.time();last=last_call.get(uid,0)
                if now-last<2:time.sleep(2-(now-last))
                last_call[uid]=time.time();send(peer,ai_response(uid,text or 'Продолжи.'))
            except Exception as e:
...
print('ND_VK_GATEWAY_V5_GROQ_START',json.dumps({'port':PORT,'token_present':bool(TOKEN),'groq_present':bool(GROQ_API_KEY),'model':GROQ_MODEL,'allowed_count':len(ALLOWED)}),flush=True)
