...
# V31: Safe Tool Broker execution layer. Models never receive credentials; unknown tools are denied.
broker_code=r"""
...
"""
def_marker='insert_before="def heuristic_route(text):\\n"'
if def_marker not in code:raise RuntimeError('V31 insert_before marker not found')
...
if needle2 not in code:raise RuntimeError('V31 composed read-only injection marker not found')
...
if route_line not in code:raise RuntimeError('V31 ndctx line not found')
code=code.replace(route_line,route_line+chr(10)+"    toolctx=safe_tool_context(original_text,route)",1)

if_line="    if ndctx:"
if if_line not in code:raise RuntimeError('V31 if ndctx line not found')
...
if text_line not in code:raise RuntimeError('V31 ndctx text line not found')
...
code=code.replace("ND_VK_GATEWAY_V13_ND_READONLY_START","ND_VK_GATEWAY_V31_SAFE_TOOL_BROKER_START",1)
'''
outer=outer.replace(marker,marker+"\n"+extra,1)
outer=outer.replace("ND_VK_GATEWAY_V23_ND_READONLY_RANKED_START","ND_VK_GATEWAY_V31_SAFE_TOOL_BROKER_RANKED_START",1)
outer=outer.replace("ND_V23_BOOTSTRAP","ND_V31_BOOTSTRAP",1)
outer=outer.replace("ND_V23_OUTER","ND_V31_OUTER",1)
outer=outer.replace("nd_vk_gateway_v23_outer.py","nd_vk_gateway_v31_outer.py",1)
print('ND_V31_WRAPPER_READY',flush=True)
exec(compile(outer,'nd_vk_gateway_v31_loader.py','exec'))
