...
patches=[
    ...
    ("ND-Gemini-Drive-MCP/1.0", "ND-Gemini-Drive-MCP/1.1")
]
for old,new in patches:
    if src.count(old)!=1:
        raise RuntimeError('v08 qualification patch anchor mismatch: '+old[:100]+' count='+str(src.count(old)))
    src=src.replace(old,new,1)
print('ND_GEMINI_DRIVE_E2E_V08_FIX_READY '+json.dumps({'drive_target':'127.0.0.1:3302/drive/invoke','contract':'X-ND-Bridge-Key + {tool,args}','temporary_qualification':True}),flush=True)
exec(compile(src,'nd_meta_vk_multiplex_front_v08_gemini_drive_e2e_fix_runtime.py','exec'))
