...
marker="src=src.replace(final_target,meta_inject,1)\n"
if outer.count(marker)!=1:
    raise RuntimeError('pass2 meta injection marker not found exactly once')

patch=r'''
# ---- ND Meta pass-2 Direct MCP + GitHub bootstrap patch ----
runtime_anchor="class H(BaseHTTPRequestHandler):\n"
if src.count(runtime_anchor)!=1:
    raise RuntimeError('pass2 runtime class anchor not found exactly once')
...
def meta_mcp_selftest_once():
    ...
    try:
        ...
        c3,s=_meta_mcp_post_local({'jsonrpc':'2.0','id':3,'method':'tools/call','params':{'name':'meta_status','arguments':{}}})
        tools=(((t.get('result') or {}).get('tools')) or [])
        structured=((s.get('result') or {}).get('structuredContent') or {})
...
"""
src=src.replace(runtime_anchor,runtime_globals+runtime_anchor,1)

post_anchor="    def do_POST(self):\n        p=self.path.split('?',1)[0]\n"
if src.count(post_anchor)!=1:
    raise RuntimeError('pass2 do_POST anchor not found exactly once')
...
"""
src=src.replace(post_anchor,mcp_method,1)

server_anchor="ThreadingHTTPServer(('0.0.0.0',PORT),H).serve_forever()"
if src.count(server_anchor)!=1:
    raise RuntimeError('pass2 server anchor not found exactly once')
src=src.replace(server_anchor,"threading.Thread(target=meta_github_bootstrap_once,daemon=True).start()\nthreading.Thread(target=meta_mcp_selftest_once,daemon=True).start()\n"+server_anchor,1)

print('ND_META_PASS2_PATCH_READY '+json.dumps({'direct_mcp':True,'github_bootstrap':bool(os.environ.get('ND_META_GITHUB_BOOTSTRAP_REV','').strip()),'browser':False},ensure_ascii=False),flush=True)
'''

outer=outer.replace(marker,marker+patch,1)
print('ND_META_PASS2_OUTER_READY',flush=True)
exec(compile(outer,'nd_meta_pass2_inner_wrapper_v01_outer.py','exec'))
