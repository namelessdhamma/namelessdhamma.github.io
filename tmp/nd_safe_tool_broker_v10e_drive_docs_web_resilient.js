console.log("ND_DRIVE_V10E_WEB_RESILIENT_WRAPPER_START");
...
if(!patched.includes(ddInvoke))throw new Error('v10e runtime invoke marker missing');
...
if(!patched.includes(ddCat))throw new Error('v10e runtime catalog marker missing');
...
if(!patched.includes(ddServe))throw new Error('v10e runtime serve marker missing');
const probe=String.raw\`
console.log("ND_DRIVE_DOCS_V10E_START",JSON.stringify({writable_ids:DRIVE_DOCS_WRITABLE.size,tools:7}));
setTimeout(async()=>{
 ...
 try{
  ...
  console.log("ND_DRIVE_DOCS_V10E_PROBE",JSON.stringify({ok:true,elapsed_ms:Date.now()-t,title:d.title,revision_id:d.revision_id,text_chars:d.text.length,drive_version:c.drive_version,modified_time:c.modified_time,revision_match:c.docs_revision_id===d.revision_id}));
 }catch(e){console.error("ND_DRIVE_DOCS_V10E_PROBE",JSON.stringify({ok:false,error:String(e).slice(0,500)}));}
},5000);
\`;
...
console.log('ND_DRIVE_V10E_OUTER_READY',JSON.stringify({outer_chars:outer.length,has_patch_logic:outer.includes('v10e runtime invoke marker'),has_docs_read:outer.includes('docs_read')}));
