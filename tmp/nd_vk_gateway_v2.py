...
state={'ok':True,'phase':'starting','group_id':None,'callback_server_id':None,'callback_registered':False,'callback_configured':False,'last_error':None,'last_event_type':None,'last_event_group_id':None,'last_event_user_id':None,'last_event_at':None}

def cleanerr(e):
    s=str(e)
    if TOKEN:s=s.replace(TOKEN,'[redacted]')
    s=s.replace(PATH_KEY,'[path-key]')
    return s[:700]
...
def register():
    gid=resolve_gid();state['group_id']=gid;state['phase']='checking_callback'
    ss=servers(gid);sid=None
    for s in ss:
        if (s.get('title') or '').startswith('ND Gateway') or (s.get('url') or '').startswith(PUBLIC_BASE+'/vk/callback'):
            sid=int(s['id']);break
    if sid is None:
        ...
        vk('groups.editCallbackServer',{'group_id':gid,'server_id':sid,'url':CALLBACK_URL,'title':'ND Gateway','secret_key':''})
...
class H(BaseHTTPRequestHandler):
    ...
    def out(self,code,body,ctype='application/json; charset=utf-8'):
        raw=(json.dumps(body,ensure_ascii=False) if isinstance(body,(dict,list)) else str(body)).encode();self.send_response(code);self.send_header('Content-Type',ctype);self.send_header('Content-Length',str(len(raw)));self.end_headers();self.wfile.write(raw)
    def do_GET(self):
    ...
    def do_POST(self):
        p=self.path.split('?',1)[0]
        if p!=CALLBACK_PATH:
            print('VK_REJECT_PATH',json.dumps({'path_len':len(p),'expected_len':len(CALLBACK_PATH)}),flush=True);self.out(404,'not found','text/plain; charset=utf-8');return
        try:
            n=int(self.headers.get('Content-Length','0'));b=json.loads(self.rfile.read(n).decode() or '{}')
        except Exception as e:
            print('VK_BAD_JSON',cleanerr(e),flush=True);self.out(400,'bad request','text/plain; charset=utf-8');return
        typ=b.get('type');gid=b.get('group_id');state['last_event_type']=typ;state['last_event_group_id']=gid;state['last_event_at']=int(time.time())
        print('VK_EVENT',json.dumps({'type':typ,'group_id':gid,'expected_group_id':state.get('group_id')}),flush=True)
        if typ=='confirmation':
            try:
        ...
        peer=m.get('peer_id');uid=m.get('from_id');text=(m.get('text') or '').strip();state['last_event_user_id']=uid
        ...
        def reply():
            ...
            except Exception as e:state['last_error']=cleanerr(e);print('VK_MESSAGE_ERROR',state['last_error'],flush=True)
        threading.Thread(target=reply,daemon=True).start()

print('ND_VK_GATEWAY_V2_START',json.dumps({'port':PORT,'token_present':bool(TOKEN),'callback_url_present':bool(CALLBACK_URL)}),flush=True)
