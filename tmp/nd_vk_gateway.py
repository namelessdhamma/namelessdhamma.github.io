...
OLD_URL=PUBLIC_BASE+'/vk/callback' if PUBLIC_BASE else ''
state={'ok':True,'phase':'starting','token_present':bool(TOKEN),'public_base_present':bool(PUBLIC_BASE),'group_id':None,'callback_server_id':None,'callback_registered':False,'callback_configured':False,'last_error':None,'last_event_type':None,'last_event_at':None,'startup_attempts':0}
...
def register():
    state['startup_attempts']+=1; state['phase']='resolving_group'
    gid=resolve_gid();state['group_id']=gid
    if not CALLBACK_URL:raise RuntimeError('public callback URL missing')
    state['phase']='checking_callback'; sid=None
    ss=servers(gid)
    for s in ss:
        u=(s.get('url') or '').rstrip('/')
        if u==CALLBACK_URL.rstrip('/'):
            sid=int(s['id'])
        elif OLD_URL and u==OLD_URL.rstrip('/'):
            try: vk('groups.setCallbackSettings',{'group_id':gid,'server_id':int(s['id']),'api_version':V,'message_new':0})
            except Exception: pass
    if sid is None:
        state['phase']='adding_callback'
        r=vk('groups.addCallbackServer',{'group_id':gid,'url':CALLBACK_URL,'title':'ND Gateway2'})
        sid=int(r.get('server_id') if isinstance(r,dict) else r)
    state['callback_server_id']=sid;state['callback_registered']=True
    time.sleep(1);state['phase']='configuring_callback'
