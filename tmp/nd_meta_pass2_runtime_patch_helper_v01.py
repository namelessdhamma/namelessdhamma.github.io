...
def meta_mcp_selftest_once():
    ...
    time.sleep(8)
    try:
        c1,_=_meta_mcp_post_local({'jsonrpc':'2.0','id':1,'method':'initialize','params':{'protocolVersion':'2025-06-18','capabilities':{},'clientInfo':{'name':'ND Meta Selftest','version':'1.0'}}})
...
def patch_linear_wrapper_source(src):
    ...
    lines = [
        ...
        "if s.count(runtime_anchor)!=1: raise RuntimeError('pass2 helper class anchor mismatch')",
        ...
        "if s.count(post_anchor)!=1: raise RuntimeError('pass2 helper do_POST anchor mismatch')",
        ...
        "if s.count(server_anchor)!=1: raise RuntimeError('pass2 helper server anchor mismatch')",
        ...
        "print('ND_META_PASS2_PATCH_HELPER_READY '+json.dumps({'direct_mcp':True,'github_bootstrap':bool(os.environ.get('ND_META_GITHUB_BOOTSTRAP_REV','').strip()),'browser':False},ensure_ascii=False),flush=True)",
        runtime_exec,
    ]
