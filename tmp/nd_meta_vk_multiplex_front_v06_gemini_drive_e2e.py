...
methods=r'''    def gemini_token_active(self):
    ...
    def gemini_drive_tool(self,tool,args):
        ...
        req=urllib.request.Request(
            ...
            headers={'Authorization':'Bearer '+QSTASH_TOKEN,'Content-Type':'application/json','Accept':'application/json','User-Agent':'ND-Gemini-Drive-MCP/1.0'})
    ...
    def gemini_drive_mcp(self):
        ...
        if method=='initialize':
            self.send_json(200,{'jsonrpc':'2.0','id':mid,'result':{'protocolVersion':str(((msg.get('params') or {}).get('protocolVersion')) or '2025-06-18'),'capabilities':{'tools':{'listChanged':False}},'serverInfo':{'name':'nd-gemini-drive-mcp','version':'1.0.1'},'instructions':'Private read-only MCP over the existing ND Google Drive authority/search broker. No writes or mutations.'}}); return
        ...
        if method=='tools/call':
            ...
            try: out=self.gemini_drive_tool(name,args); err=False
            except Exception as e: out={'ok':False,'error':clean(e)}; err=True
    ...
    def gemini_e2e_selftest(self):
        if not GEMINI_SELFTEST_TOKEN:
            self.send_json(503,{'ok':False,'error':'selftest_disabled'}); return
        payload={'jsonrpc':'2.0','id':'railway-e2e-selftest','method':'tools/call','params':{'name':'gemini_memory_query','arguments':{'query':'From live ND memory in Google Drive, report only the current StateHead status, Capability Registry version, and component count. Do not use prior/model memory.','max_tokens':2048,'temperature':0}}}
        try:
            req=urllib.request.Request(GEMINI_MCP_URL,data=json.dumps(payload).encode('utf-8'),method='POST',headers={'Authorization':'Bearer '+GEMINI_SELFTEST_TOKEN,'Content-Type':'application/json','Accept':'application/json','MCP-Protocol-Version':'2025-06-18','User-Agent':'ND-Gemini-Drive-E2E/1.0'})
            ...
            self.send_json(200,{'ok':bool(out.get('live_drive_retrieval')),'provider':out.get('provider'),'model':out.get('model'),'role':out.get('role'),'content':str(out.get('content') or '')[:2000],'tools_used':out.get('tools_used') or [],'live_drive_retrieval':bool(out.get('live_drive_retrieval')),'context_packet_required':out.get('context_packet_required'),'tool_access_mode':out.get('tool_access_mode'),'write_authority':out.get('write_authority')}); return
        except Exception as e:
...
get_inject="        if path=='/gemini/drive/e2e-selftest':\n            self.gemini_e2e_selftest(); return\n        if path=='/gemini/drive/health':\n            self.send_json(200,{'ok':True,'service':'nd-gemini-drive-mcp','mode':'PRIVATE_READ_ONLY','tools':['nd_authority','google_drive_search'],'auth':'nd-gemini-mcp bearer introspection','mcp':'/gemini/drive/mcp','selftest_enabled':bool(GEMINI_SELFTEST_TOKEN)}); return\n"+health_anchor
if src.count(health_anchor)!=1:
    raise RuntimeError('gemini drive GET anchor mismatch')
src=src.replace(health_anchor,get_inject,1)

print('ND_GEMINI_DRIVE_E2E_V06_READY '+json.dumps({'mcp':'/gemini/drive/mcp','selftest':'/gemini/drive/e2e-selftest','bounded':True}),flush=True)
exec(compile(src,'nd_meta_vk_multiplex_front_v06_gemini_drive_e2e_runtime.py','exec'))
