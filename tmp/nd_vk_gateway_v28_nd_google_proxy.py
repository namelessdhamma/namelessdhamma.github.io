...
if def_marker not in code:raise RuntimeError('V28 insert_before marker not found')
...
if needle not in code:raise RuntimeError('V28 ro_code marker not found')
...
if gline not in code:raise RuntimeError('V28 github pieces line not found')
...
tline="threading.Thread(target=nd_ro_probe,daemon=True).start()"
if tline in code:code=code.replace(tline,tline+chr(10)+"threading.Thread(target=google_gateway_probe,daemon=True).start()",1)
code=code.replace("ND_VK_GATEWAY_V13_ND_READONLY_START","ND_VK_GATEWAY_V28_ND_GOOGLE_PROXY_START",1)
'''
outer=outer.replace(marker,marker+"\n"+extra,1)
outer=outer.replace("ND_VK_GATEWAY_V23_ND_READONLY_RANKED_START","ND_VK_GATEWAY_V28_ND_GOOGLE_PROXY_RANKED_START",1)
outer=outer.replace("ND_V23_BOOTSTRAP","ND_V28_BOOTSTRAP",1)
outer=outer.replace("ND_V23_OUTER","ND_V28_OUTER",1)
outer=outer.replace("nd_vk_gateway_v23_outer.py","nd_vk_gateway_v28_outer.py",1)
print('ND_V28_WRAPPER_READY',flush=True)
exec(compile(outer,'nd_vk_gateway_v28_loader.py','exec'))
