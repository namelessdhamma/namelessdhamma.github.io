...
const compact=String.raw`async function webCurrent(q){
  ...
  const concise=raw.slice(0,520);
  const normal=("Today is "+today+". Search the current web. Prefer official/primary and recent sources. Return direct source URLs and dates. Request: "+concise).slice(0,700);
  const tiny=("Web search now. Date "+today+". Give current answer with 3 direct source URLs and dates. Q: "+raw.slice(0,260)).slice(0,380);
  const plans=[
    {model:"groq/compound-mini",query:normal,custom:true},
    {model:"groq/compound",query:normal,custom:true},
    {model:"groq/compound",query:tiny,custom:false}
  ];
  let lastErr="";
  for(const plan of plans){
    const payload={model:plan.model,messages:[{role:"user",content:plan.query}],citation_options:"enabled"};
    if(plan.custom)payload.compound_custom={tools:{enabled_tools:["web_search"]}};
    for(let attempt=0;attempt<2;attempt++){
      let r,t;
      try{
        r=await fetch("https://api.groq.com/openai/v1/chat/completions",{method:"POST",headers:{Authorization:"Bearer "+GROQ_API_KEY,"Content-Type":"application/json","Groq-Model-Version":"latest","User-Agent":"nd-safe-tool-broker/9b"},body:JSON.stringify(payload)});
        t=await r.text();
      }catch(e){lastErr="groq network: "+String(e).slice(0,220);if(attempt===0){await new Promise(res=>setTimeout(res,900));continue;}break;}
      if(r.ok){
        ...
        return {brief:String(msg.content||"").trim().slice(0,6500),source_urls:urls,tool_calls:exec.length,verified_search:true,model:plan.model,query_chars:plan.query.length,mutations:false};
      }
      lastErr="groq "+r.status+": "+t.slice(0,300);
      const retryable=(r.status===413||r.status===422||r.status===429||r.status===498||r.status===499||r.status>=500);
      if(retryable&&attempt===0){await new Promise(res=>setTimeout(res,r.status===429?1500:800));continue;}
      break;
    }
  }
...
patched=patched.replace('service:"ND Safe Tool Broker v2 + Google Authority",mode:"READ_ONLY",mutations:false','service:"ND Safe Tool Broker v9b + Father Comment Sandbox",mode:"BOUNDED_WRITE_SANDBOX",canonical_mutations:false,sandbox_mutations:true');
patched=patched.replace('console.log("ND_SAFE_TOOL_BROKER_START",JSON.stringify({port:PORT,statehead_id:STATEHEAD_ID,google:!!CLIENT_EMAIL&&!!PRIVATE_KEY_B64,github:!!GITHUB_TOKEN,web:!!GROQ_API_KEY,mutations:false}));','console.log("ND_SAFE_TOOL_BROKER_V9B_WEB_RESILIENT_START",JSON.stringify({port:PORT,statehead_id:STATEHEAD_ID,google:!!CLIENT_EMAIL&&!!PRIVATE_KEY_B64,github:!!GITHUB_TOKEN,web:!!GROQ_API_KEY,canonical_mutations:false,sandbox_mutations:true,father_root:FATHER_ROOT_ID,storage:"OWNER_OWNED_DRIVE_COMMENT_APPEND_ONLY_LEDGERS"}));');
