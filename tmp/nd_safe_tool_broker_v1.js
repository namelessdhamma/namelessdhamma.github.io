...
async function githubRead(q){
  ...
  const ts=qterms(q), headers={Authorization:"Bearer "+GITHUB_TOKEN,Accept:"application/vnd.github+json","User-Agent":"nd-safe-tool-broker/1.0"};
  const paths=[];
  if(ts.length){
    try{
      const sq=encodeURIComponent(ts.slice(0,4).join(" ")+" repo:"+repo);
      const r=await fetch("https://api.github.com/search/code?q="+sq,{headers});
      if(r.ok){const j=await r.json();for(const it of (j.items||[])){const p=it.path||"";if(safeGitPath(p)&&!paths.includes(p))paths.push(p);if(paths.length>=4)break;}}
    }catch{}
  }
  if(!paths.length&&repo.endsWith("nameless-dhamma-vault"))paths.push("00 СЕЙЧАС.md","01 ТЕМЫ.md","02 РЕШЕНИЯ.md","03 ИЗМЕНЕНИЯ.md");
  const out=[];
  for(const p of paths.slice(0,4)){
    try{
      const r=await fetch("https://api.github.com/repos/"+repo+"/contents/"+p.split("/").map(encodeURIComponent).join("/")+"?ref=main",{headers});
      if(!r.ok)continue;const j=await r.json();const txt=Buffer.from((j.content||"").replace(/\n/g,""),"base64").toString("utf8");
      if(txt.trim())out.push({repo,path:p,text:txt.slice(0,6000)});
    }catch{}
  }
  return {repo,results:out,mutations:false};
}
...
async function handler(req){
  ...
  if(u.pathname==="/health")return Response.json({ok:true,service:"ND Safe Tool Broker + Google Authority",mode:"READ_ONLY",mutations:false});
