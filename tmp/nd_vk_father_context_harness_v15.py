...
def build_context_envelope(uid, *, nd_query='ND current architecture and relevant project context'):
    ...
    for s in (nd.get('sources') or [])[:20]:
        ...
        if authority not in ('AUTHORITATIVE','CANONICAL'):
            continue
        sources.append({'source_ref':str(s.get('source_ref') or '')[:500],'title':str(s.get('title') or '')[:500],'authority':authority})
    envelope={'schema':'nd.vk_father.context_envelope.v1','uid':str(int(uid)),'conversation':history,'nd_context':{'text':nd_text,'sources':sources,'statehead_status':nd.get('statehead_status'),'registry_version':nd.get('registry_version')},'policy':{'conversation_authority':'ND_FATHER_WORKSPACE','nd_authority_allowlist':['AUTHORITATIVE','CANONICAL'],'non_auth_injection':False,'production_mutation':False}}
...
def build_response_messages(uid,user_text,*,nd_query='ND current architecture and relevant project context'):
    """Compose a provider-ready bounded message list without sending it anywhere."""
    env=build_context_envelope(uid,nd_query=nd_query)
    nd=env.get('nd_context') or {}
    lines=['Governed ND context. Use only according to stated authority/provenance.']
    ...
    if nd.get('text'): lines.append(str(nd.get('text'))[:MAX_ND_CHARS])
    messages=[{'role':'system','content':'You are the VK interface of Nameless Dhamma. Default to Russian. Do not invent project state or sources.'},{'role':'system','content':'\n'.join(lines)[:12000]}]
    messages.extend(_clip_messages(env.get('conversation') or []))
    messages.append({'role':'user','content':str(user_text)[:MAX_USER_CHARS]})
    receipt={'schema':'nd.vk_father.response_harness.v1','uid':env.get('uid'),'history_turns':len(env.get('conversation') or []),'nd_source_count':len(nd.get('sources') or []),'nd_authorities':sorted(set(str(x.get('authority') or '') for x in (nd.get('sources') or []) if isinstance(x,dict))),'statehead_status':nd.get('statehead_status'),'registry_version':nd.get('registry_version'),'non_auth_injection':bool((env.get('policy') or {}).get('non_auth_injection')),'production_mutation':False}
...
def qualification_summary(envelope):
    ...
    return {'schema':envelope.get('schema'),'uid':envelope.get('uid'),'conversation_turns':len(conv),'conversation_chars':sum(len(str(x.get('content') or '')) for x in conv if isinstance(x,dict)),'nd_context_chars':len(str(nd.get('text') or '')),'nd_sources':len(nd.get('sources') or []),'nd_authorities':sorted(set(str(x.get('authority') or '') for x in (nd.get('sources') or []) if isinstance(x,dict))),'statehead_status':nd.get('statehead_status'),'registry_version':nd.get('registry_version'),'non_auth_injection':bool(envelope.get('policy',{}).get('non_auth_injection'))}
