...
def find_special_mailbox(imap, flag):
    try:
        typ, boxes = imap.list()
        if typ != "OK":
            return None
...
def main():
    ...
    # Scan the whole mailbox, not only Inbox:
    # Gmail's \\All mailbox covers Inbox, archived mail and category tabs.
    # Spam is outside All Mail, so it is scanned separately.
    all_mail = find_all_mailbox(imap)
    primary_mailbox = all_mail or "INBOX"

    primary_messages = fetch_today(imap, primary_mailbox, start_utc, end_utc)
    messages = list(primary_messages)

    spam_messages = []
    spam = find_spam_mailbox(imap)
    if spam:
        spam_messages = fetch_today(imap, spam, start_utc, end_utc)
        messages.extend(spam_messages)

    # Deduplicate defensively by Message-ID/date/from/subject in case a provider
    # exposes overlapping special-use folders.
    deduped = []
    seen = set()
    for m in messages:
        key = m["message_id"] or (m["date"], m["from"].lower(), m["subject"].lower())
        if key in seen:
            continue
        seen.add(key)
        deduped.append(m)
    ...
    log(
        f"status=ok date={local_date} mailbox={primary_mailbox!r} "
        f"all_mail_scanned={len(primary_messages)} spam_scanned={len(spam_messages)} "
        f"scanned_unique={len(messages)} "
