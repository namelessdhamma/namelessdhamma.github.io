...
# V37: Safe Tool Broker execution layer. Models never receive credentials; unknown tools are denied.
broker_code=r"""
...
def nd_safe_self_probe():
    time.sleep(12)
    tests=[
        ('nd_authority','Read current ND architecture, StateHead, Registry and System Skill.'),
        ('google_drive_search','True Research current canonical skill'),
        ('github_read','current ND architecture decisions Recent Changes'),
        ('web_current','Find the latest official OpenAI ChatGPT release-notes update. Use official OpenAI source and return direct URL/date.')
    ]
    for tool,q in tests:
        t=time.time()
        try:
            r=broker_invoke(tool,q)
            meta={'tool':tool,'ok':True,'elapsed_ms':int((time.time()-t)*1000),'mutations':False}
            if tool=='nd_authority' and isinstance(r,dict):
                meta.update({'statehead_status':r.get('statehead_status'),'registry_version':r.get('registry_version'),'component_count':r.get('component_count'),'pieces':len(r.get('pieces') or [])})
            elif tool=='google_drive_search' and isinstance(r,dict):
                meta['results']=len(r.get('results') or [])
            elif tool=='github_read' and isinstance(r,dict):
                meta.update({'repo':r.get('repo'),'results':len(r.get('results') or []),'wikilinks':r.get('followed_wikilinks')})
            elif tool=='web_current' and isinstance(r,dict):
                meta.update({'source_urls':len(r.get('source_urls') or []),'verified_search':r.get('verified_search',bool(r.get('source_urls'))),'model':r.get('model')})
            print('ND_SAFE_SELF_PROBE',json.dumps(meta,ensure_ascii=False),flush=True)
        except Exception as e:
            print('ND_SAFE_SELF_PROBE',json.dumps({'tool':tool,'ok':False,'error':cleanerr(e),'mutations':False},ensure_ascii=False),flush=True)

"""
def_marker='insert_before="def heuristic_route(text):\\n"'
if def_marker not in code:raise RuntimeError('V37 insert_before marker not found')
...
if needle2 not in code:raise RuntimeError('V37 composed read-only injection marker not found')
code=code.replace(needle2,"src=src.replace(insert_before,ro_code+google_ro_code+broker_code+insert_before,1)",1)

thread_marker='src=src.replace("ND_VK_GATEWAY_V12_RESEARCH_INTEGRITY_START", "ND_VK_GATEWAY_V13_ND_READONLY_START",1)'
if thread_marker not in code:raise RuntimeError('V37 V13 start marker not found')
thread_patch='src=src.replace("threading.Thread(target=startup,daemon=True).start()", "threading.Thread(target=startup,daemon=True).start()\\nthreading.Thread(target=nd_safe_self_probe,daemon=True).start()",1)'
code=code.replace(thread_marker,thread_marker+chr(10)+thread_patch,1)
...
if route_assign not in code:raise RuntimeError('V37 original_text marker not found')
code=code.replace(route_assign,route_assign+chr(10)+override_code,1)

route_line="    ndctx=nd_read_context(original_text,route)"
if route_line not in code:raise RuntimeError('V37 ndctx line not found')
...
if_line="    if ndctx:"
if if_line not in code:raise RuntimeError('V37 if ndctx line not found')
...
if text_line not in code:raise RuntimeError('V37 ndctx text line not found')
...
cmd_new=cmd_old+chr(10)+"""                if text in ('/ктоя','/id'):
                ...
                if text in ('/источники','/sources'):
                    send(peer,'Источники чтения: ND authority и Google Drive; GitHub и Obsidian; Яндекс.Диск для книг; актуальный веб-поиск. Все подключения к ND только на чтение; изменения запрещены.');return
                if text in ('/статус','/status'):
                    send(peer,'Система работает. AI: Groq + резерв OpenRouter. ND: чтение Google Drive/StateHead/Registry + GitHub/Obsidian. Книги: Яндекс.Диск, только чтение. Интернет: source-backed web research. Изменения ND запрещены. Режим: %s.'%mode_by_uid.get(uid,'auto'));return
                if text=='/tools':
...
code=code.replace("ND_VK_GATEWAY_V13_ND_READONLY_START","ND_VK_GATEWAY_V37_SAFE_TOOL_BROKER_START",1)
'''
outer=outer.replace(marker,marker+"\n"+extra,1)
outer=outer.replace("ND_VK_GATEWAY_V23_ND_READONLY_RANKED_START","ND_VK_GATEWAY_V37_SAFE_TOOL_BROKER_RANKED_START",1)
outer=outer.replace("ND_V23_BOOTSTRAP","ND_V37_BOOTSTRAP",1)
outer=outer.replace("ND_V23_OUTER","ND_V37_OUTER",1)
outer=outer.replace("nd_vk_gateway_v23_outer.py","nd_vk_gateway_v37_outer.py",1)
print('ND_V37_WRAPPER_READY',flush=True)
exec(compile(outer,'nd_vk_gateway_v37_loader.py','exec'))
