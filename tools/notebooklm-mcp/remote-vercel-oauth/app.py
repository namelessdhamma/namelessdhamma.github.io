...
# Doctor relay is intentionally preview-only. Production continues to expose
# only the normal OAuth-protected MCP surface.
...
async def doctor_health(request: Request) -> JSONResponse:
    ...
    return JSONResponse(
        {
            "ok": True,
            "service": "nd-notebooklm-vercel-doctor-relay",
            "version": "1.0.0",
            "provider": "NotebookLM",
...
async def doctor_notebooks(request: Request) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse(
            {"ok": False, "operation": "notebooks_list", "error": str(exc)[:1600]},
            status_code=502,
        )
...
async def doctor_sources(request: Request) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse(
            {"ok": False, "operation": "sources_list", "error": str(exc)[:1600]},
            status_code=502,
        )
...
async def doctor_ask(request: Request) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse(
            {"ok": False, "operation": "chat_ask", "error": str(exc)[:1600]},
            status_code=502,
        )
