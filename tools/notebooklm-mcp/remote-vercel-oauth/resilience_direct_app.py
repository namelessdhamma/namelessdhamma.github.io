...
BACKEND = os.environ.get("NOTEBOOKLM_BACKEND", "web").strip().lower() or "web"
RUNTIME_NAME = os.environ.get("ND_NOTEBOOKLM_RUNTIME_NAME", "nd-notebooklm-render-direct").strip()
HOME = Path(os.environ.get("ND_NOTEBOOKLM_RUNTIME_HOME", "/tmp/nd-notebooklm-render"))
...
def verify_oidc(request: Request) -> dict | None:
    ...
    try:
        ...
        if claims.get("actor") != _ACTOR:
            return None
        if claims.get("event_name") not in {"push", "workflow_dispatch", "schedule", "issues"}:
            return None
        return claims
    except Exception:
        return None
...
async def health(request: Request) -> JSONResponse:
    return JSONResponse({
        ...
        "bootstrap_ready": bool(BOOTSTRAP_SHARED_TOKEN),
...
async def bootstrap_exchange(request: Request) -> JSONResponse:
    auth = request.headers.get("authorization", "").strip()
    if not BOOTSTRAP_SHARED_TOKEN or auth != "Bearer " + BOOTSTRAP_SHARED_TOKEN:
    ...
    if not oauth_token:
        return bad("missing_oauth_token", 400)
    storage_path = HOME / "profiles" / "default" / "storage_state.json"
    ...
    try:
        ...
        return JSONResponse({"ok": False, "target": "render", "error": str(exc)[:1200]}, status_code=502)
    ...
    try:
        async with client_factory() as client:
            if operation == "notebook_list":
                ...
                result = {"notebook_id": nb, "source": to_jsonable(await client.sources.add_text(nb, title, body))}
