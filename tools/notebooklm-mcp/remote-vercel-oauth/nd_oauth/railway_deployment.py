...
from .full_server import create_drive_only_mcp, create_full_mcp
...
def _build_drive_only_from_environ(env: Mapping[str, str] | None = None):
    source = os.environ if env is None else env
    base_url = source.get("ND_NOTEBOOKLM_OAUTH_BASE_URL", "").strip().rstrip("/")
    if not base_url:
        raise RuntimeError("Missing ND_NOTEBOOKLM_OAUTH_BASE_URL")
    ...
    registry_store = FileOAuthStateStore(persistent_dir / "oauth-registry.json")
    transient_store = FileOAuthStateStore(persistent_dir / "oauth-transient.json")
    return create_drive_only_mcp(
        password=password,
        login_password=login_password,
        base_url=base_url,
        state_path=state_path,
...
def build_railway_app_from_environ(env: Mapping[str, str] | None = None):
    try:
        ...
        if "Missing NOTEBOOKLM_MASTER_TOKEN_B64 or ND_NOTEBOOKLM_MASTER_TOKEN_AESGCM_B64" not in str(exc):
            raise
        print("ND_RAILWAY_DRIVE_ONLY_FALLBACK master_token_unavailable", flush=True)
        mcp = _build_drive_only_from_environ(env)
    return mcp.http_app(
        path="/mcp",
        stateless_http=True,
        json_response=True,
